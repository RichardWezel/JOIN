/** Pencil icon of the edit buttons. Named per file so it cannot clash. */
const CONTACT_ICON_EDIT = `<svg width="19" height="19" viewBox="0 0 19 19" fill="none" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg">
    <path d="M2 17H3.4L12.025 8.375L10.625 6.975L2 15.6V17ZM16.3 6.925L12.05 2.725L13.45 1.325C13.8333 0.941667 14.3042 0.75 14.8625 0.75C15.4208 0.75 15.8917 0.941667 16.275 1.325L17.675 2.725C18.0583 3.10833 18.2583 3.57083 18.275 4.1125C18.2917 4.65417 18.1083 5.11667 17.725 5.5L16.3 6.925ZM14.85 8.4L4.25 19H0V14.75L10.6 4.15L14.85 8.4Z" fill="#2A3647"/>
</svg>`;

/** Trash can icon of the delete buttons. */
const CONTACT_ICON_DELETE = `<svg width="17" height="18" viewBox="0 0 17 18" fill="none" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg">
...
function contactHTML(i, currentPerson, firstLetter, secondLetter) {
    ...
    return `
        <div onclick="changeBackground(this);selectPerson(${i}); handleClick(${i})" id="persons_details" class="persons_details">
            <div class="persons" style="background-color: ${safeColor(currentPerson.name.color)};">${escapeHtml(firstLetter)}${escapeHtml(secondLetter)}</div>
            <div class="person_details">${escapeHtml(currentPerson.name.firstName)} ${escapeHtml(secondName)} <span class="you" id="youContactList${i}"></span><br><div class="email">${escapeHtml(currentPerson.mail)}</div></div>
        </div>
    `;
}
...
async function setYou_contacts() {
    await getCurrentUserIdFromServer();
    let ownIndex = person.findIndex((contact) => contact.id === currentUserId);
    if (ownIndex !== -1) {
        let you = document.getElementById(`youContactList${ownIndex}`);
        if (you) you.innerHTML = "(You)";
    }
}
...
function selectPerson(index) {
    ...
    document.getElementById('selectedPersonInitials').innerHTML = `<div class="persons" style="background-color: ${safeColor(selectedPerson.name.color)};">${escapeHtml(selectedPerson.name.firstName.charAt(0).toUpperCase() + selectedPerson.name.secondName.charAt(0).toUpperCase())}</div>`;
...
function selectPersonHTML(selectedPerson, index) {
    ...
    <div class="persons" style="background-color: ${safeColor(selectedPerson.name.color)};">${escapeHtml(selectedPerson.name.firstName.charAt(0).toUpperCase() + selectedPerson.name.secondName.charAt(0).toUpperCase())}</div>
    <div class="person_details">${escapeHtml(selectedPerson.name.firstName)} ${escapeHtml(selectedPerson.name.secondName)}
    <div onclick="openWindow()" class="button_contacts">
    <div id="editDeleteWindow" class="edit_delete_window">
    <button type="button" onclick="editPerson(${index})" aria-label="Edit contact">
        ${CONTACT_ICON_EDIT}Edit
    </button>
    <button type="button" onclick="deletePerson(${index})" aria-label="Delete contact">
        ${CONTACT_ICON_DELETE}Delete
    </button>
    </div>
...
<div class="info">
    ...
    <h4>Email</h4>
    <div class="emails">${escapeHtml(selectedPerson.mail)}</div>
    <h4>Phone</h4>
    <p>${escapeHtml(selectedPerson.phone)}</p>
    `;
}
...
async function deletePerson() {
    ...
    let id = person[index].id;
    closeContactDialog();
    await deleteContactOnServerById(id);
    await getContactsFromServer();
    person = contacts_global;
    if (person.length > 0) {
...
async function createContact(event) {
    ...
    if (nameInput && emailInput && phoneInput) {
        ...
        loadContacts();
        closeContactDialog();
        let newIndex = person.findIndex(p => p.id === created.id);
        if (newIndex !== -1) {
            selectPerson(newIndex);
            setTimeout(function () {
...
async function saveChanges() {
    ...
    if (nameInput && emailInput) {
        ...
        await updateContactOnServer(selectedPerson.id, {
            firstName,
            secondName,
            mail: emailInput,
            phone: phoneInput,
            color: selectedPerson.name.color,
        });
        await getContactsFromServer();
        person = contacts_global;
        ...
        let updatedIndex = person.findIndex(p => p.id === selectedPerson.id);
        if (updatedIndex !== -1) {
