...
/**
 * Validates that a category was picked from the dropdown.
 * categoryDropDownBtn is a <div>, not an <input>, so it has no .value to check -
 * the actual selection lives in the global pushCategory variable.
 *
 * @returns {Boolean} - Marks whether a category is selected.
 */
function validateCategory() {
  let input = document.getElementById('categoryDropDownBtn');
  let errortext = document.getElementById('validation_text_category');
  if (pushCategory === '') {
    window.location.hash = 'categoryDropDownBtn';
