...
async function renderDropList() {
  ...
  for (let i = 0; i < contacts_addTask.length; i++) {
    ...
    if ((currentUser !== '' && currentUserId !== 999 || currentUser !== 999 && currentUserId !== 999) && ownContact) {
      if (contact.name.firstName == ownContact.name.firstName &&
          contact.name.secondName == ownContact.name.secondName ||
          contact.name.firstName == ownContact.name.firstName &&
