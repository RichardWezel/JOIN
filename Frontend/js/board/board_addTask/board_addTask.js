...
function DueDateEditDialogHTML_addTask() {
    return /*html*/`
        ...
        <form>
            <input class="" onclick="setTodayAndMinDate_addTask()" onfocusout="DueDatevalidation_addTask()" placeholder="dd/mm/yyyy" id="edit_input_dueDate_addTask" type="date"  required>
        </form>
...
/**
 * Pre-fills the due date input with today's date and blocks picking a date in the past.
 */
function setTodayAndMinDate_addTask() {
    let dateInput = document.getElementById('edit_input_dueDate_addTask');
    let today = new Date;
    let day = String(today.getDate()).padStart(2, '0');
    let month = String(today.getMonth() + 1).padStart(2, '0');
    let year = today.getFullYear();
    let todayIso = `${year}-${month}-${day}`;
