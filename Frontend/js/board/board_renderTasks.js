    ...
    for (let i = 0; i < tasks.length; i++) {
        ...
        case 'toDo': 
            if(!search_content || 
                ...
                {
                    addTaskElementToBoard(toDo, task, task.id)
                }
            break;
        case 'inProgress': 
            if(!search_content || 
                ...
                task.category.toLowerCase().includes(search_content)) 
                {
                    addTaskElementToBoard(inProgress, task, task.id)
                }
            break;
        case 'awaitFeedback':
            if(!search_content || 
                ...
                task.category.toLowerCase().includes(search_content)) 
                {
                    addTaskElementToBoard(awaitFeedback, task, task.id)
                }
            break;
        case 'done':
            if(!search_content || 
                ...
                task.category.toLowerCase().includes(search_content)) 
                {
                    addTaskElementToBoard(done, task, task.id)
                }
            break;
        }
    }
...
function setColorOfCategory(taskId) {
    let category = getTaskById(taskId).category;
