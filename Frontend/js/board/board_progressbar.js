...
function renderBlueProgressbar(taskId) {
    ...
    let totalSubtasks = getTaskById(taskId).subtasks.length;
    if (totalSubtasks > 0) {
        if (amountOfOpenTasks == 0 && container.style) {
            container.style.width = '0px';
        } else if (amountOfOpenTasks == getTaskById(taskId).subtasks.length) {
...
function renderHoverTextProgressbar(taskId) {
    ...
    let subtasks_total = getTaskById(taskId).subtasks.length;
...
function renderSubtaskAmounts(taskId) {
    ...
    sub_total.innerHTML = getTaskById(taskId).subtasks.length;
}