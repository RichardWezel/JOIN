/** Trash can icon of the delete button in the task dialog. */
const ICON_DELETE = `<svg width="17" height="18" viewBox="0 0 17 18" fill="none" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg">
...
/** Pencil icon of the edit button in the task dialog. */
const ICON_EDIT = `<svg width="19" height="19" viewBox="0 0 19 19" fill="none" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg">
...
function taskDialogHTML(task, taskId) {
    return `
        <div class="category_close">
                <div onclick="doNotClose(event)" id="task_category_dialog" class="task_category">${escapeHtml(task.category)}</div>
                <button type="button" onclick="closeDialog(${taskId})" class="close" aria-label="Close task details">
                    <div class="line horizontal"></div>
                    <div class="line vertical"></div>
                </button>
            </div>
            <!-- title -->
            <div onclick="doNotClose(event)" class="task_title_dialog">${escapeHtml(task.title)}</div>
            <!-- description -->
            <div onclick="doNotClose(event)" class="task_description_dialog">${escapeHtml(task.description)}</div>
            <!-- Due date -->
            ...
            <div onclick="doNotClose(event)" class="priority_dialog">
                Priority: <span >${escapeHtml(task.priority)}</span><div id="prio_image"></div>
            </div>
            <!-- assigned to -->
            ...
            <div onclick="doNotClose(event)" class="delete_and_edit_section">
                <div class="delete_and_edit">
                    ...
                    <button type="button" onclick="deleteTask(${taskId})">
                        <div id="delete_img_dialog">${ICON_DELETE}</div>
                        <div class="delet_text">Delete</div>
                    </button>

                    <div class="line_task"></div>

                    <button type="button" id="edit_img_dialog" onclick="openTaskEdit(${taskId})">
                        <div id="delete_img_dialog">${ICON_EDIT}</div>
                        <div class="delet_text">Edit</div>
                    </button>
...
function setColorOfCategoryInDialog(i) {
    let category = getTaskById(i).category;
...
async function renderAssigedToDialog(taskId) {
    ...
    if (task.contacts.length == 0) {
        ...
        for (let i = 0; i < task.contacts.length; i++) {
            ...
            let ownContact = getCurrentUserContact();
            if (currentUser !== '' && currentUserId !== 999 && ownContact) {
                if (contact.firstName == ownContact.name.firstName &&
                    contact.secondName == ownContact.name.secondName) {
...
function AssigedToDialogHTML(contact, taskId, contactId) {
    return `
        <tr>
            <td id="taskdetailscontact${taskId}${contactId}" class="member_cycle pos1">${escapeHtml(contact.firstName.charAt(0) + contact.secondName.charAt(0))}</td>
            <td class="member_name_assiged_to">${escapeHtml(contact.firstName)} ${escapeHtml(contact.secondName)} <span id="you_task_details${taskId}${contactId}" class="you"></span></td>
        </tr>
    `;
}
...
function renderPriorityDialog(taskId) {
    let priority = getTaskById(taskId).priority;
...
function subtaskDialogHTML(taskId, subtaskId, subtaskContent) {
    return `
        <div class="Subtasks">
            ...
            <span>${escapeHtml(subtaskContent)}</span>
        </div>
    `;
}
...
async function changeSubtaskStatus(taskId, subtaskId) {
    let subtask = getTaskById(taskId).subtasks[subtaskId];
    subtask.done = !subtask.done;
    await toggleSubtaskOnServer(subtask.id, subtask.done);
    renderSubtasksDialog(taskId);
...
function renderSubtaskImage(taskId, subtaskId) {
    ...
    let status = getTaskById(taskId).subtasks[subtaskId].done;
    switch (status) {
        case true:
          container.innerHTML = `<button type="button" class="checkbox1" onclick="changeSubtaskStatus(${taskId}, ${subtaskId})" aria-pressed="true" aria-label="Subtask done"><img src="assets/img/check_button_checked.svg" alt="" aria-hidden="true"></button>`;
          break;
        case false:
            container.innerHTML = `<button type="button" class="checkbox1" onclick="changeSubtaskStatus(${taskId}, ${subtaskId})" aria-pressed="false" aria-label="Subtask done"><img src="assets/img/check_button_unchecked.svg" alt="" aria-hidden="true"></button>`;
          break;
    }
}
...
async function deleteTask(taskId) {
    await deleteTaskOnServerById(taskId);
